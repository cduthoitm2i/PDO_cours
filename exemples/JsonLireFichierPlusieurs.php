...
//$contenuFichier = file_get_contents("ville.json");
$contenuFichier = file_get_contents("villes.json");
// Affichage du contenu du fichier
echo $contenuFichier;

// json_decode(chaine, tableau_associatif = true)
// chaine: It is encoded string which must be UTF-8 encoded data
$tObjects = json_decode($contenuFichier, true);

echo "<hr>";
// Boucle sur le tableau des éléments du tableau d’objets PHP
for ($i = 0; $i < count($tObjects); $i++) {
    // Affichage des valeurs des clés de chaque élément
    echo $tObjects[$i]["id_pays"] . " : " . $tObjects[$i]["nom_ville"] . " : " . $tObjects[$i]["cp"] . " : " . $tObjects[$i]["site"] . " : " . $tObjects[$i]["photo"] . "<br>";
}
?>